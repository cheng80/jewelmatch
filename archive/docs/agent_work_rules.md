...
- 유효한 진행 기록은 TimeUp 진입 또는 일시정지 나가기에서 제출한다. 일시정지 나가기는 제출 완료를 기다린다. TimeUp 나가기는 진입 시 보낸 제출을 기다리지 않는다. 클라이언트 계약은 [`tools/ranking_client_contract.md`](tools/ranking_client_contract.md)를 본다.
