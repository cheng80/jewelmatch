...
- [x] 같은 실행의 FPS `AVG / LOW / GAP` 화면 수치 기록
- [ ] BGM·효과음 ON을 유지한 수동 플레이 30초에서 FPS와 시스템 추적 동시 측정
...
| 2026-08-15 | 게임 루프 상시 할당 축소, 사운드 켬 | Simple 자동 30초 | 55.0 | 41.7 | 139ms | 22 | 20 | 총 잠재 정지 73회. 100ms 이상 WebContent 정지 20회 모두 GPU·`mediaremoted` 정지와 동시 발생했고, 근처에서 GC 표본이 확인된 것은 4회라 오디오 경로가 우선 병목 후보 |
...
- FPS: 30초 누적 `AVG 55.0 / LOW 41.7 / GAP 139ms`
...
- 다음 단일 테스트: BGM·효과음 ON을 유지한 수동 플레이 30초에서 `AVG / LOW / GAP`과 오디오·WebContent 정지를 동시에 기록
