...
구현(2026-09-24): 마무리 중에는 일시정지를 열 수 없고, 앱이 백그라운드로 가면 남은 발동을 즉시 계산해 TimeUp으로 간다. 판 종료 확정은 `_finalizeRound()` 한 곳이며 기록 반영과 round_end가 여기서 한 번 일어난다. 결과 패널과 제출의 1900ms는 TimeUp 진입부터 센다.
