...
| 항목 | 입력값 또는 준비 상태 |
|---|---|
| 앱 이름 | `Stone Match`, 콘솔에서 수정 가능 |
| appName | 출시 전 확정, 등록 뒤 수정과 삭제 불가 |
| 앱 유형 | 게임 |
...
| 카테고리 | 콘솔 게임 장르에서 퍼즐을 우선 선택, 실제 선택지 확인 후 확정 |
| 검색 키워드 | `매치3`, `퍼즐`, `보석`, `스톤매치`, `캐주얼게임`, `타임어택` |
| 리더보드 | 점수 단위와 정렬 방향을 확정한 뒤 콘솔에서 설정 |

### 이미지 자료

| 자료 | 규격 | Stone Match 준비 기준 |
|---|---|---|
| 앱 로고 | 600×600, 배경색이 있는 PNG, 투명 배경 불가 | 앱 아이콘 원본에서 별도 제작, 다크 모드 가독성 확인 |
| 썸네일 | 1932×828, PNG | 가로형 핵심 플레이 화면과 짧은 로고로 새 제작 |
| 세로 스크린샷 | 선택, 636×1048 PNG 최소 3장 | 타이틀, 게임 플레이, 모드 또는 랭킹 화면 |
| 가로 스크린샷 | 선택, 1504×741 PNG 최소 1장 | 가로 게임 화면을 출시할 때만 사용 |
