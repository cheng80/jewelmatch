...
- 실험 보석(PLAN-008, 타임 모드): Time 보석은 보석 오른쪽 아래에 칸 45퍼센트 시계 배지와 "+5", Multiplier 보석은 가운데에 칸 55퍼센트 배율 판과 "×N"(지우면 얻는 배율). 배율 m이 2 이상이면 점수 오른쪽에 금색 "×m"이 뜨고 오를 때 팝한다(reduced motion은 팝 없음). URL 실험 판의 결과 화면은 랭킹 대신 "URL 실험 설정으로 한 판이라 랭킹에 올리지 않았습니다"를 보인다. 7번째 색은 흰 돌이다
