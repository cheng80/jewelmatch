...
- 주요 컴포넌트: 로고, 무한/레벨/타임/랭킹 원형 버튼, 설정/튜토리얼 아이콘, 하단 버전
...
- 특수 보석과 HUD glow: 특수 발동의 번개, 별, 폭발, 하이퍼 광륜과 목표, 콤보, 타임바, 아이템 광륜은 기존 색, 위치, 히트 영역을 유지한 채 baked glow로 표시한다. 특수 연출은 사라진 뒤 잔상을 남기지 않는다.
...
- HUD 왕관은 타임 top1 이름/점수 요약, 탭 시 목록
