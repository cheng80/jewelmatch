...
- 외부 표시명은 `Stone Match` 또는 제출물 표기인 `STONE MATCH`를 사용한다. `Jewel Match`는 남아 있는 내부 식별자를 설명할 때만 사용한다.
...
- NAS 배포는 `nas-web-build` 스킬과 기존 배포 스크립트를 사용한다. 상세 웹 빌드 규칙은 [`tools/web_build.md`](tools/web_build.md)를 따른다.
