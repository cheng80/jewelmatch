...
이 문서는 [`item_slot_market_plan.md`](item_slot_market_plan.md)의 실행용 체크리스트다. 기획 원칙은 플랜 문서를 기준으로 하고, 실제 작업은 이 문서의 단계별 체크박스를 갱신한다.
...
- [ ] 1차에서는 아이템 수량/인벤토리/영속 저장을 만들지 않는다.
- [ ] `itemTargeting` 상태를 게임 또는 보드 입력 계층에 추가한다.
- [ ] 대상 선택 중 일반 스왑 입력을 막는다.
- [ ] 대상 선택 취소 경로를 제공한다.
- [ ] 대상 선택 완료/취소/실패 후 원래 시간 흐름으로 복귀한다.

### UI

- [ ] 하단에 8개 테스트 슬롯 영역을 만든다.
- [ ] 1줄: 룬 망치, 고대 폭탄, 토르 망치, 하이퍼 큐브.
- [ ] 2줄: 프리즘 변환, 운명의 셔플, 타임 슬립, 힌트+.
- [ ] 슬롯은 보드 하단 공간을 침범하지 않게 반응형 배치한다.
- [ ] 비활성 아이템은 눌러도 보드 상태가 변하지 않게 한다.
- [ ] 타임 슬립은 시간 없는 모드에서 비활성 처리한다.
- [ ] 힌트+는 무한 모드에서 비활성 처리한다.

### 보드 효과

- [ ] 룬 망치: 선택 셀 1개 제거.
- [ ] 고대 폭탄: 선택 셀 중심 3×3 제거.
- [ ] 토르 망치: 선택 셀 기준 `GemKind.star`와 같은 십자 제거.
- [ ] 하이퍼 큐브: 선택 보석 색상과 같은 non-hyper 보석 전체 제거.
- [ ] 하이퍼 큐브가 특수 보석을 선택하면 1차에서는 `kind`가 아니라 `color`만 사용한다.
- [ ] 프리즘 변환: 선택 보석 색 변경 UI와 변경 후 매치 체크를 구현한다.
- [ ] 운명의 셔플: 특수 보석 위치를 보존하고 일반 보석만 섞는다.
- [ ] 운명의 셔플 후 최소 1개 유효 이동을 보장한다.
- [ ] 타임 슬립: `hasTimedClock`이 true인 모드에서만 시간을 추가한다.
- [ ] 힌트+: 제한 모드 힌트 잔량을 증가시킨다.

### 테스트·QA

- [ ] 각 아이템 효과 단위 테스트를 추가한다.
- [ ] 가장자리/모서리에서 룬 망치, 고대 폭탄, 토르 망치 범위가 안전하게 잘린다.
- [ ] 하이퍼 큐브가 일반 보석 선택 시 같은 색 전체 제거를 수행한다.
- [ ] 하이퍼 큐브가 특수 보석 선택 시 색상만 참조한다.
- [ ] 운명의 셔플 후 특수 보석 좌표가 유지된다.
- [ ] 대상 선택 중 타임바 시간이 줄지 않는다.
- [ ] 대상 선택 중 일반 스왑이 발생하지 않는다.
...
- 지급 아이템은 앱 실행 세션 동안만 유지한다.
- 실제 슬롯 제한과 다음 스테이지 loadout을 검증한다.
...
- [ ] 2차에서는 슬롯 해금 UI를 mock 또는 고정 상태로 둔다.
...
- [ ] 2차에서는 클리어 시에만 지급한다.
- [ ] 한 스테이지 무료 보상은 최대 2개로 제한한다.
- [ ] 여러 조건 충족 시 희귀도/우선순위가 높은 아이템을 먼저 지급한다.

### 결과 화면·다음 스테이지

- [ ] 클리어/결과 화면에 지급 아이템 목록을 표시한다.
- [ ] 다음 스테이지 진입 시 `RunInventory`의 수량을 사용할 수 있다.
- [ ] 아이템 사용 시 임시 수량이 1 감소한다.
...
- [ ] 보상 조건별 단위 테스트를 추가한다.
- [ ] 보상 최대 2개 제한 테스트를 추가한다.
- [ ] 중복 지급 방지 테스트를 추가한다.
- [ ] 임시 인벤토리 수량 차감 테스트를 추가한다.
- [ ] 앱 재시작 없이 다음 스테이지에서 보상 아이템 사용 가능.
- [ ] 앱 재시작 후 2차 임시 인벤토리는 유지되지 않아도 된다.
