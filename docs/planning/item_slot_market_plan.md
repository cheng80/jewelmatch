...
상세 구현 체크리스트는 [`item_slot_market_implementation_checklist.md`](item_slot_market_implementation_checklist.md)를 기준으로 갱신한다. 스테이지 보상 지급 규칙은 [`stage_reward_system.md`](stage_reward_system.md)를 기준으로 한다.
...
| 2차 | 스테이지 종료 보상과 런 인벤토리 | `MatchBoardGameStats` 기반 보상 판정, 결과 화면 보상 표시, 4×2 인벤토리, 레벨클리어 창 전용 장착 변경, `RunInventory`, `StageLoadout`, 현재 런 동안 유지되는 보유량, 슬롯 제한 검증 | 별도 마켓 UI, 영속 데이터, 광고, 과금 |
...
0차는 아이템 슬롯에 들어가기 전에 힌트의 기본 가치를 만들기 위한 선행 작업이다. 무한 모드는 연습모드 성격이므로 힌트 제한을 두지 않고, 타임/레벨 모드부터 제한형 힌트를 적용한다. 1차의 목적은 경제 시스템이나 보상 루프가 아니라 “각 아이템이 실제 플레이에서 쓸 만한가”를 빠르게 확인하는 것이다. 아직 유저가 붙지 않았고 웹게임 테스트가 중심이므로, 슬롯 제한과 인벤토리를 넣기 전에 하단에 8개 아이템 슬롯을 두 줄로 노출해 사용감부터 본다. 2차부터 슬롯 제한, 런 인벤토리, 스테이지 종료 보상 루프를 붙인다.
...
1차는 정식 경제/인벤토리 기능이 아니라 테스트 UI다. 목적은 아이템 8종의 효과, 선택 흐름, 시간 처리, 보드 밸런스 영향을 빠르게 보는 것이다.
...
- 대상 칸 선택이 필요한 아이템은 `itemTargeting` 상태에서 일반 스왑 입력을 막되, 시간은 계속 흐르게 둔다.
...
| 조건 | 지급 아이템 | 수량 | 의도 |
|------|-------------|------|------|
| `removedGems >= 80` 또는 `matchGroups >= 14` 또는 `score / targetScore >= 1.05` 그리고 `removedGems >= 60` | 룬 망치 | 1 | 꾸준한 제거/매치 성과 보상 |
| `board.maxCombo >= 4` 또는 `removedSpecialGems >= 3` 또는 `score / targetScore >= 1.10` 그리고 `board.maxCombo >= 3` | 고대 폭탄 | 1 | 연쇄와 특수 보석 활용 보상 |
| `specialGemsCreated >= 4` 또는 `specialCreatedByKind[GemKind.hyper] >= 1` 또는 `score / targetScore >= 1.15` 그리고 `specialGemsCreated >= 2` | 프리즘 변환 | 1 | 특수 보석 제작 중심 플레이 보상 |
| `specialGemsActivated >= 2` 또는 `specialActivatedByKind[GemKind.star] >= 1` 또는 `score / targetScore >= 1.12` 그리고 `specialGemsActivated >= 1` | 토르 망치 | 1 | 특수 효과를 실제로 발동한 보상 |
| `validSwaps`가 1~12 그리고 `removedGems >= 45` | 타임 슬립 | 1 | 적은 스왑으로 클리어한 효율 보상 |
| `matchGroups >= 18` 그리고 (`board.maxCombo >= 3` 또는 `specialGemsCreated >= 2`) | 운명의 셔플 | 1 | 많은 매치와 추가 성과가 함께 나온 보상 |
| 제한 모드에서 힌트 미사용 클리어 | 힌트+ | 1 | 자력 플레이 보상 |
| `removedGems >= 120` 그리고 `board.maxCombo >= 5`, 또는 `score / targetScore >= 1.25` 그리고 `board.maxCombo >= 4` 그리고 `specialGemsCreated >= 5` | 하이퍼 큐브 | 1 | 대량 제거·고콤보·특수 제작이 결합된 고급 보상 |
| 위 조건을 하나도 만족하지 못한 클리어 | 토르 망치 | 1 | 기준 미달 클리어의 최소 지급 보장 |
...
- 대상 칸 선택이 필요한 아이템은 선택 완료 또는 취소 전까지 일반 스왑 입력을 막되 시간은 계속 흐르게 둔다.
