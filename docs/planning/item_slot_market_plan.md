...
- 0차 힌트 제한, 1차 아이템 효과 검증, 2차 스테이지 보상·런 인벤토리·다음 스테이지 로드아웃 편집까지 구현됐다.
- 2차 기준 구현은 `19ad065`(보상·인벤토리), `31e4969`(오버레이 레이아웃), `6f5b40e`(모바일 HUD 정책)와 현재 코드·테스트를 근거로 한다.
- 기존 에이전트 작업 계획의 유효한 범위·제약·검증 조건은 이 문서와 체크리스트, [`stage_reward_system.md`](stage_reward_system.md)에 통합했다.
- 3차 영속 인벤토리 이후 단계는 후보이며 아직 구현하지 않았다. 사용자 지표와 배포 정책이 생기기 전에는 착수하지 않는다.
...
- 레벨 모드 힌트 수량은 현재 런 동안 누적한다. 앱 재시작 후 영속화는 3차 저장 모델과 함께 검토한다.
...
1차는 정식 경제/인벤토리 기능이 아니라 테스트 UI다. 목적은 아이템 8종의 효과, 선택 흐름, 시간 처리, 보드 밸런스 영향을 빠르게 보는 것이다. 현재 8개 슬롯 HUD는 심플 모드의 일반 debug/profile 개발 빌드에서만 유지하고, 일반 release에서는 숨긴다. `QA_SPECIAL_EFFECTS=true` 빌드는 8개 아이템 대신 특수 효과 6종 패널을 표시한다.
...
1차 UI는 최종 슬롯 정책과 다르다. 최종 구조는 최대 4개 슬롯, 초기 2개 슬롯이다. 현재 진행 모드는 `StageLoadout`, 심플 모드 debug/profile은 개발 아이템 8개, 심플 모드 release와 타임 모드는 하단 패널 없음, 명시적 QA 빌드는 특수 효과 6종 패널 정책을 사용한다.

## 슬롯 규칙

- 슬롯 최대 개수는 4개다.
- 신규 유저의 기본 오픈 슬롯은 2개다.
- 진행 모드는 `StageLoadout` 4칸 인벤토리 패널을 사용한다.
- 심플 모드는 일반 debug/profile에서 개발 아이템 8개, 일반 release에서 패널 없음, `QA_SPECIAL_EFFECTS=true`에서 특수 효과 6종 패널을 사용한다.
- 타임 모드는 빌드 유형과 관계없이 하단 패널을 표시하지 않는다.
...
이 슬롯 규칙은 진행 모드의 현재 구조다. 새 런은 룬 망치와 고대 폭탄 각 1개로 시작하고, 이후 장착은 레벨클리어 창의 `StageInventory`에서 다음 스테이지용 로드아웃을 편집한다.
...
| 12스테이지 클리어 + 최근 3스테이지 중 보상 2개 이상 획득 1회 | 슬롯 4 | 해금 | 같은 방식으로 인벤토리 창을 열고 4번 슬롯 해금을 명시한다. |
...
레벨클리어 전용 `StageInventory`는 2차에 구현됐다. 타이틀·준비 화면 진입, 영속 저장, 장착 해제까지 포함한 확장 인벤토리는 3차 범위다.

### 화면 진입 위치

- 2차: 레벨클리어 결과창의 인벤토리 아이콘으로만 진입.
- 3차 후보: 타이틀 화면의 `인벤토리`, 스테이지 시작 전 `아이템 선택` 진입.
- 4차 후보: 게임 오버/클리어 화면의 `아이템 보충` 진입.
...
아이템별 레어도 요약은 [스테이지 보상 체계](stage_reward_system.md)의 `아이템 레어도` 표를 기준으로 삼는다.
...
3.5차에서는 GA/Firebase를 붙이기 전에 내부 이벤트 로거를 먼저 둔다. 목표는 외부 SDK 선택이 아니라 “무엇을 측정할지”를 고정하는 것이다.
...
- 레벨 모드 힌트 잔량은 현재 런 동안 누적하고, 영속 저장은 3차에서 검토한다.
...
- 레벨클리어 창에는 받은 보상을 먼저 표시하고, 인벤토리 버튼을 눌렀을 때만 4×2 인벤토리를 열어 다음 스테이지의 열린 슬롯 장착 항목을 설정한다.
...
- 보상 판정은 `StageRewardEvaluator`로 보드 로직에서 분리하고, 조건표는 `stage_reward_system.md`에서 관리한다.
...
- 6·12스테이지 조건의 슬롯 3·4 해금은 구현했다. 마켓, 코인 구매, 앱 재시작 후 유지 저장은 구현하지 않는다.
- QA 브리지는 런 인벤토리, 로드아웃, 최근 보상, claim key를 노출한다.
